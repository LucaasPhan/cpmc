...
import { Animator, batch, Fade, MoveOut, MoveIn, ScrollPage } from "react-scroll-motion";
import {font} from "./_styles/font";


const Core = ({title: title = false, switchSide: switchSide = false, position, positionVN, name, nickname: nickname = "", content, image: image = "/Gigachad.jpg"}: {title?: boolean, switchSide?: boolean, position?: string, positionVN?: string, name?: string, content?: string, image?: string, nickname?: string}) => {
    return (
        <section id="core">
            <ScrollPage>
                {title ? (
